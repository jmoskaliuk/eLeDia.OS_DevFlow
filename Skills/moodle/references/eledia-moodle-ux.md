...
For plugins targeting Moodle 5.2+ with React surfaces, prefer `@moodlehq/design-system` (`--mds-*` tokens, `<Button label=… variant=…>`) — full reference in `references/moodle-design-system.md`, which wins on any MDS conflict. The `--lh-*` layer remains canonical for 4.5–5.1 support and for eLeDia brand semantics; hybrid pattern: derive `--lh-*` surfaces from `--mds-*` where available, keep brand + plugin-semantic colors as `--lh-*`.
